...
type Provider = "google" | "github" | "microsoft" | "tiktok" | "twitter" | "discord";
...
const providerConfig: Record<Provider, { name: string; icon: React.ReactNode }> = {
  google: {
    name: "Google",
    icon: (
...
export function OAuthProviders({ className, mode = "login" }: OAuthProvidersProps) {
  const [loadingProvider, setLoadingProvider] = useState<Provider | "passkey" | null>(null);
  ...
  const handleOAuthLogin = async (provider: Provider) => {
    ...
    try {
      ...
      toast.error(`Failed to ${mode === "login" ? "sign in" : "sign up"} with ${providerConfig[provider].name}`);
